...
#Region "Private-Members"
        ...
        Private _Header As String = "[DatabaseWrapper.SqlServer] "
...
#Region "Constructors-and-Factories"
        ...
        Public Sub New(ByVal settings As DatabaseSettings)
            ...
            End If
            If _Settings.Type <> DbTypes.SqlServer Then
        ...
        Public Sub New(ByVal serverIp As String, ByVal serverPort As Integer, ByVal username As String, ByVal password As String, ByVal instance As String, ByVal database As String)
...
#Region "Public-Methods"
        ...
        Public Function ListTables() As List(Of String)
            ...
            Dim result As DataTable = Query(SqlServerHelper.LoadTableNamesQuery(_Settings.DatabaseName))
        ...
        Public Function DescribeTable(ByVal tableName As String) As List(Of Column)
            ...
            Dim result As DataTable = Query(SqlServerHelper.LoadTableColumnsQuery(_Settings.DatabaseName, tableName))
            If result IsNot Nothing AndAlso result.Rows.Count > 0 Then
                For Each currColumn As DataRow In result.Rows
                    ...
                    If currColumn("CONSTRAINT_NAME") IsNot Nothing AndAlso currColumn("CONSTRAINT_NAME") IsNot DBNull.Value AndAlso Not String.IsNullOrEmpty(currColumn("CONSTRAINT_NAME").ToString()) Then
                        If currColumn("CONSTRAINT_NAME").ToString().ToLower().StartsWith("pk") Then
        ...
        Public Function [Select](ByVal tableName As String, ByVal indexStart? As Integer, ByVal maxResults? As Integer, ByVal returnFields As List(Of String), ByVal filter As Expr) As DataTable
            If String.IsNullOrEmpty(tableName) Then
                Throw New ArgumentNullException(NameOf(tableName))
            End If
            If indexStart IsNot Nothing Then
                Throw New ArgumentException("For SQL Server, use the Select API including result order when using a starting index.")
            End If
            Return Query(SqlServerHelper.SelectQuery(tableName, indexStart, maxResults, returnFields, filter, Nothing))
        End Function
        ...
        Public Function [Select](ByVal tableName As String, ByVal indexStart? As Integer, ByVal maxResults? As Integer, ByVal returnFields As List(Of String), ByVal filter As Expr, ByVal resultOrder() As ResultOrder) As DataTable
            If String.IsNullOrEmpty(tableName) Then
                Throw New ArgumentNullException(NameOf(tableName))
            End If
            If indexStart IsNot Nothing AndAlso (resultOrder Is Nothing OrElse resultOrder.Length < 1) Then
                Throw New ArgumentException("For SQL Server, result order must be populated when using a starting index.")
            End If
        ...
        Public Function Insert(ByVal tableName As String, ByVal keyValuePairs As Dictionary(Of String, Object)) As DataTable
            ...
            For Each currKvp As KeyValuePair(Of String, Object) In keyValuePairs
                ...
                outputkeys &= SqlServerHelper.PreparedOutputFieldName(currKvp.Key)
                ...
                If currKvp.Value IsNot Nothing Then
                    ...
                    ElseIf TypeOf currKvp.Value Is Byte() Then
                        vals &= "0x" & BitConverter.ToString(DirectCast(currKvp.Value, Byte())).Replace("-", "")
                    Else
            ...
            Return Query(SqlServerHelper.InsertQuery(tableName, keys, vals, outputkeys), keys, vals, kvpfieldtype)
        ...
        Public Function Update(ByVal tableName As String, ByVal keyValuePairs As Dictionary(Of String, Object), ByVal filter As Expr) As DataTable
            ...
            For Each currKvp As KeyValuePair(Of String, Object) In keyValuePairs
                ...
                If currKvp.Value IsNot Nothing Then
                    If TypeOf currKvp.Value Is DateTime OrElse TypeOf currKvp.Value Is DateTime? Then
                        keyValueClause &= SqlServerHelper.PreparedFieldName(currKvp.Key) & "='" & DbTimestamp(DirectCast(currKvp.Value, DateTime)) & "'"
                    ElseIf TypeOf currKvp.Value Is DateTimeOffset OrElse TypeOf currKvp.Value Is DateTimeOffset? Then
                        keyValueClause &= SqlServerHelper.PreparedFieldName(currKvp.Key) & "='" & DbTimestampOffset(DirectCast(currKvp.Value, DateTime)) & "'"
                    ElseIf TypeOf currKvp.Value Is Integer OrElse TypeOf currKvp.Value Is Long OrElse TypeOf currKvp.Value Is Decimal Then
                        keyValueClause &= SqlServerHelper.PreparedFieldName(currKvp.Key) & "=" & currKvp.Value.ToString()
                    ElseIf TypeOf currKvp.Value Is Byte() Then
                        keyValueClause &= SqlServerHelper.PreparedFieldName(currKvp.Key) & "=" & "0x" & BitConverter.ToString(DirectCast(currKvp.Value, Byte())).Replace("-", "")
                    Else
                        If Helper.IsExtendedCharacters(currKvp.Value.ToString()) Then
...
#Disable Warning BCCA2100 ' Review SQL queries for security vulnerabilities
                        Dim sda As New SqlDataAdapter(query_Conflict, conn)
#Enable Warning BCCA2100 ' Review SQL queries for security vulnerabilities
                ...
                Else
                    ...
                    CmdSql = New SqlCommand(query_Conflict, DbConn)
                    With CmdSql
        ...
        Public Function Exists(ByVal tableName As String, ByVal filter As Expr) As Boolean
            If String.IsNullOrEmpty(tableName) Then
                Throw New ArgumentNullException(NameOf(tableName))
            End If
            Dim result As DataTable = Query(SqlServerHelper.ExistsQuery(tableName, filter))
            If result IsNot Nothing AndAlso result.Rows.Count > 0 Then
                Return True
            End If
            Return False
        ...
        Public Function Count(ByVal tableName As String, ByVal filter As Expr) As Long
            If String.IsNullOrEmpty(tableName) Then
                Throw New ArgumentNullException(NameOf(tableName))
            End If
            Dim result As DataTable = Query(SqlServerHelper.CountQuery(tableName, _CountColumnName, filter))
        ...
        Public Function Sum(ByVal tableName As String, ByVal fieldName As String, ByVal filter As Expr) As Decimal
            ...
            End If
            Dim result As DataTable = Query(SqlServerHelper.SumQuery(tableName, fieldName, _SumColumnName, filter))
